...
class SSNBlock(nn.Module):
    ...
    def __init__(self, args, state_layer_idx):
        ...
        self.supralinear_act = lambda x: args.supra_k * torch.nn.ReLU()(x)**args.supra_n
        ...
        if state_layer_idx == 0:
            for i, inp_size in enumerate(inp_sizes):
                ...
                gamma_init = torch.distributions.gamma.Gamma(gamma_alpha,
                                                             gamma_beta)
                e_conv.weight.data = torch.nn.Parameter(gamma_init.sample() * args.weights_mag_scale)
                ...
                gamma_init = torch.distributions.gamma.Gamma(gamma_alpha,
                                                             gamma_beta)
                i_conv.weight.data = torch.nn.Parameter(-gamma_init.sample() * args.weights_mag_scale)
            ...
            for i, inp_size in enumerate(inp_sizes):
                ...
                gamma_init = torch.distributions.gamma.Gamma(gamma_alpha,
                                                             gamma_beta)
                ee_conv.weight.data = torch.nn.Parameter(gamma_init.sample() * args.weights_mag_scale)
                ...
                gamma_init = torch.distributions.gamma.Gamma(gamma_alpha,
                                                             gamma_beta)
                ei_conv.weight.data = torch.nn.Parameter(gamma_init.sample() * args.weights_mag_scale)
                ...
                if state_layer_idx == self.inp_idxs[i]:  # i.e. if self connection
                    ...
                    ie_conv.weight.data = torch.nn.Parameter(
                        -gamma_init.sample() * args.weights_mag_scale)
                    ...
                    self.convs.update({ie_label: ie_conv,
                                       ii_label: ii_conv})
                    #TODO remove centre weight by setting to 0 if state_layer_idx == self.inp_idx[i]
    ...
    def forward(self, layer_states, inps):
        ...
        if self.state_layer_idx == 0:
            ...
            for k, nrn_type_block in enumerate(full_pre_quad_outs):
                nrn_type_block = torch.stack(nrn_type_block)
                full_pre_quad_outs[k] = torch.sum(nrn_type_block, dim=0)
...
class SSNInterBlock(nn.Module):
    """"""
    def __init__(self, args, state_layer_idx, in_channels, out_channels, kernel_size,
                 only_conv=False, only_conv_t=False, padding=None,
                 layer_norm=False, weight_norm=False):
        super().__init__()
        self.args = args
        ...
        inp_idxs         = args.arch_dict['mod_connect_dict'][state_layer_idx]
        inp_state_shapes = [self.args.state_sizes[j] for j in inp_idxs]
        in_channels = sum([inp_state_shapes])
        ...
        self.block = nn.Sequential(self.interp,
                                   self.conv_layer,
                                   self.act)
