...
class SSNEBM(BaseModel):
    ...
    def forward_ff_dynamics(self, states, energy_calc=False):
        ...
        for i, (layer_states, net, layer_biases) in enumerate(zip(states,
                                                   self.quadratic_nets,
                                                   self.biases)):
            ...
            post_inp_idxs = self.args.arch_dict['mod_connect_dict'][i]
            pos_inp_states = [states[j] for j in post_inp_idxs]
            # pos_inp_states = [[self.st_act(state) for state in layer_states]
            #                   for layer_states in pos_inp_states]
            ...
            for ei_ind, state in enumerate(layer_states):
                if i == 0 and ei_ind > 0:
                    break #i.e. don't try to update inh states for layer 0 because there is none

                network_terms = full_pre_quadr_out[ei_ind] + layer_biases[ei_ind].weight.view(full_pre_quadr_out[ei_ind].shape[1:])
                #network_terms = full_pre_quadr_out + bias
